...
function renderizarTela(scripts, avisos) {
    ...
    if (corpoTabela) {
        corpoTabela.innerHTML = scripts.map(item => `
            <tr class="hover:bg-gray-50 transition">
                ...
                <td class="p-3 border-b text-gray-600">${item.setor}</td>
    ...
    if (containerAvisos) {
        containerAvisos.innerHTML = avisos.length > 0 
            ...
            : '<p class="text-gray-400 text-center py-4">Nenhum aviso no momento.</p>';
    }
...
async function enviarSugestao() {
    ...
    if (!input || input.value.trim() === "") return alert("Digite algo!");
    const { error } = await _supabase.from('sugestoes').insert([{ texto: input.value, data: new Date().toLocaleString('pt-BR') }]);
    if (!error) { alert("Sugestão enviada!"); input.value = ""; }
}

_supabase.channel('mudancas-db').on('postgres_changes', { event: '*', schema: 'public', table: 'scripts' }, carregarDadosDoSupabase).on('postgres_changes', { event: '*', schema: 'public', table: 'avisos' }, carregarDadosDoSupabase).subscribe();
